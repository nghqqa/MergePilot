...
1. **（可选）MinioTicketStore**：P-1 若用户认董方案 B，实现 MinIO 票据存储适配层（语义单测不变）+ 门页只读展示；
2. **R6 方案细化**：token usage 源二选一（外部 OTel collector 查询 vs worker 本地台账）——需用户选路；
3. **待用户输入**：D-1/D-2/D-3（审批动作集/审批人/TTL）、P-1（存储落点）、预算金额、R1-R4 授权批复——之后执行真实集成轮（M1 场景 3/6/7 实证 + 副本同步 + 真 manifest 落盘验证）。
