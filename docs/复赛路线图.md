...
| B4 审批票据 + action_outbox | 设计 v2 ✅ + B4a/a.1/a.2/a.3 ✅ | L2 动作审批闭环。**设计 v2**([docs/M3-B4-审批票据与Action-Outbox设计.md](M3-B4-审批票据与Action-Outbox设计.md))已通过。**B4a**:DB schema + 10 个 l2_* SECURITY DEFINER 函数 + 两个 EXECUTE-only 账号。**B4a.1**:函数 owner 改 mergepilot_l2_owner + payload/binding 一致性 + pending_list 全 payload + 漂移收敛。**B4a.2**:恢复 pgvector owner(allowlist)+ payload/hash/TTL 封闭 + GRANT 顺序 + 精确 ACL + 右方向 membership。**B4a.3**:owner/REVOKE 循环改完整 regprocedure 签名(防同名 overload)+ pgvector 经 pg_depend 定位 + JSON 类型校验(jsonb_typeof)+ 收敛 owner membership。61/61 PASS。详见 [标签SHA映射.md](标签SHA映射.md)。待做:B4b(Gateway 边界)。 |
