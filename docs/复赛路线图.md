...
| B3 不可变审计强化 | ✅ closed `m3b-b3-closed` + `m3b-b3.1-closed` | INSERT-only 账号 + 写 fail-closed + correlation_id 事件。**B3.1**:幂等补 `phase`/`decision` CHECK 约束(CREATE IF NOT EXISTS 不改已有表);角色脚本始终收敛授权(env 存在也重建);`run-policy-gateway.sh` `set -e`+ON_ERROR_STOP 严格 fail-fast(init 失败不替换 gateway);权限逐项断言;密码不打印前缀+轮换;revoke PUBLIC TEMP。19/19 PASS |
