...
| B4 审批票据 + action_outbox | 设计稿(待复审) | L2 动作(merge/revert/close)的人工审批闭环。**详见 [docs/M3-B4-审批票据与Action-Outbox设计.md](M3-B4-审批票据与Action-Outbox设计.md)**。核心:绑定来源 = GitHub(Controller 读回,不信任 LLM 自报);`policy_gateway_l2` 只 EXECUTE 受约束 DB 函数(无表级 UPDATE);`approvals` 为执行权威(PENDING→APPROVED→EXECUTING→USED,含 UNKNOWN 对账),`policy_action_outbox` 为派发意图(不加 EXECUTING);Controller/CLI/Gateway 三方职责分离。暂未实现,待 6 个开放问题拍板。 |
