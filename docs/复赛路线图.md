...
| B2 最小权限矩阵 | ✅ closed `m3b-b2-closed` + `m3b-b2-hardened` | policy.yaml deny-by-default + fixer 写 arg 校验 + L2 占位。**B2.1 hardening**(代码审查后):修 3 绕过(from_branch、coordinator 继承 fix 过权、repo allowlist 只覆盖写)+ update_pull_request 字段白名单(title/body 才放行,state→L2)+ 5 工具全禁(create/fork repo、search_repos/users、assign_copilot)+ repo allowlist 覆盖所有读 + search scope 约束。22/22 PASS |
| B3 不可变审计强化 | 待做 | gateway 独立 INSERT-only DB 账号;**写操作/L2 动作审计不可用时 fail-closed**(B1 是 fail-open) |
| B4 审批票据 + action_outbox | 待做 | PENDING→APPROVED→EXECUTING→USED;票据绑 run_id+action+repo+pr+target_branch+head_sha+expires_at;policy_action_outbox 原子领取;网络超时→UNKNOWN 先查 GitHub |
