...
| B4 审批票据 + action_outbox | 设计稿 **v2**(待复审) | L2 动作(merge/close)的人工审批闭环。**详见 [docs/M3-B4-审批票据与Action-Outbox设计.md](M3-B4-审批票据与Action-Outbox设计.md)**。v2 闭合:v1 通配 PR 发现不可行(改分页+本地过滤)、Controller 须持 coordinator token、票据绑完整 canonical_payload、claim 一次 CAS 全校验、execution_id 对账、outbox DISPATCHED lease 恢复、账号真 EXECUTE-only、SECURITY DEFINER 硬化、task_runs 加 APPROVAL_PENDING、revert 走"建 PR→正常 merge"。暂未实现,待复审 + 3 个小问题(执行超时/merge_method/lease 时长)。 |
