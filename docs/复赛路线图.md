...
下一主线为 **初赛材料收尾 → SAST 路径统一 → migration runner（MIG-B4-001）→ OTel/SLS / 生产 HiClaw（`hiclaw_live`）→ N≥20 Benchmark**。其中生产 HiClaw 受 D2B-3 BLOCKED_UPSTREAM 阻塞，当前以受控隔离测试栈证据替代。
...
- **边界**：设计冻结 ≠ 实现。`hiclaw_live=false` 与 `UPSTREAM_BLOCKED.md` 状态**不变**。
...
**诚实边界（D2B-3B1.x 未覆盖）**：
- 本轮为**离线 stub 验收**，**未启动真实 dockerd / AgentTeams / HiClaw**。Windows host 无 AF_UNIX，传输测试用 socketpair broker（AF_INET）运行**相同 production handler 代码**；POSIX 部署用真实 AF_UNIX 文件系统 socket。
- marker 写入未在真实 `/etc/hiclab/` 落地（仅 stub 测试了逻辑契约）。
- `UPSTREAM_BLOCKED.md` 状态**未改**（仍 BLOCKED_UPSTREAM / option b）；`hiclaw_live` 保持 **false**。
- **labels 缺失不是兼容模式**：所有经 proxy 创建的新容器都带权威 labels；旧版无 label 容器必须清理或重新创建，不能自动信任。

#### D2B-3B2 隔离环境 Smoke（**待授权**，未开始）

1. [ ] 真实隔离栈（MergePilot-Test 独立 dockerd + VHDX）部署 proxy + 真 controller。
2. [ ] proxy HTTP round-trip（含 streaming、hijack）端到端验证。
3. [ ] marker 真实 `/etc/hiclab/` 落地 + PID liveness 验证。
4. [ ] controller `AGENTTEAMS_PROXY_SOCKET` 重定向验证。
5. [ ] 三场景：正向 create→start→inspect→stop→rm、负向 deny、绕过尝试。
6. [ ] residue=0、secret=0、`hiclaw_live=false`（隔离栈，非生产）。
7. [ ] **仅在上述全过后**，评估 D2B-3C（生产部署 + `hiclaw_live` 评估）。

> D2B-3B2 未完成前，D2B-3 `BLOCKED_UPSTREAM`（option b）不变：Manager auto-create FORBIDDEN，仅 `create_hardened_worker.sh` 手动路径允许。
