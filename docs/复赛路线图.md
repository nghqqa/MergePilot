...
- **M4-B ⏳ 技术复审通过、待发布**：`diff-parse`（真实 unified diff → 结构化变更上下文）与 `risk-classify`（确定性、建议型、只升不降 L0/L1/L2，版本化规则、无 Nacos/author_trust/policy.yaml）已落地并各达 DoD；经两轮独立负向审计（P1/P2 + R2）逐项修复（含上述 common-runtime 加固、quoted 转义双引号路径、布尔谓词 `const:true` fail-closed 等）；**96 项测试两轮稳定，M4-A 回归 75/75×2，M3 零 diff**。证据 `evidence/m4/m4b/`。待按 `common 加固 → delivery → tag m4b-diff-risk-closed → docs → push` 发布。
- 待办：SASTScan / TestRunner / PRLifecycle / CaseRetrieval 等其余核心 Skill（M4-C 起）。
