...
| B3 不可变审计强化 | ✅ closed `m3b-b3-closed` | gateway 用独立 `policy_gateway_audit` 账号(INSERT-only,拒 SELECT/UPDATE/DELETE/TRUNCATE);**写操作审计 fail-closed**(INTENT 必须先持久化才调 GitHub,否则 `AUDIT_UNAVAILABLE` 拒绝且无 GitHub 副作用);读/list/auth-deny 保持 fail-open;追加式 `correlation_id`+`phase=INTENT/RESULT/ERROR`(永不 UPDATE);多仓库启动断言。11/11 PASS(含独立坏-DSN gateway 验证无副作用) |
