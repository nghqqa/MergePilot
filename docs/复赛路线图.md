...
| B4 审批票据 + action_outbox | 设计 v2 ✅ + B4a ✅ `m3b-b4a-closed` + B4a.1 ✅ | L2 动作审批闭环。**设计 v2**([docs/M3-B4-审批票据与Action-Outbox设计.md](M3-B4-审批票据与Action-Outbox设计.md))已通过。**B4a**(commit `b4e649e`):DB schema + 10 个 l2_* SECURITY DEFINER 函数 + 两个 EXECUTE-only 账号。**B4a.1**:函数 owner 改 mergepilot_l2_owner(NOLOGIN 隔离真成立)+ payload/binding 一致性校验 + pending_list 返回完整 payload(审批可见 merge_method)+ 角色脚本漂移收敛(REVOKE all l2_* 再 GRANT)+ reconcile_executing 120s 约束 + p_effect_applied + 真并发 claim 测试。44/44 PASS。待做:B4b(Gateway 边界)、B4c(Controller 绑定/drain/对账)、B4d(approve CLI)、B4e(E2E + 崩溃恢复)。 |
