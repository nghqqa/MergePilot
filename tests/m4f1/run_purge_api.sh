...
IMG="pgvector/pgvector@sha256:a36250871de0833b8757561c72f2477ef1ddd1101afa4e617fb552e0de514c6b";UNIQ="$$-$(date +%s)";DB="m4f1-qa-$UNIQ";LABEL="m4f1-qa-$UNIQ";TMP_ROOT="$(mktemp -d)";rc=1
...
for suite in producer_api_audit complete_api_audit purge_api_audit;do if ! docker exec -i "$DB" psql -X -U mergepilot -d app -v ON_ERROR_STOP=1 <"$SQLDIR/$suite.sql">"$TMP_ROOT/$suite" 2>&1;then cat "$TMP_ROOT/$suite";exit 1;fi;done
grep -q 'QA-SET PASS: 9' "$TMP_ROOT/purge_api_audit";echo 'PURGE API PASS ids=9';rc=0
