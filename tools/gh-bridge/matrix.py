...
def preflight(timeout_s=8):
    """启动前凭证预检(2026-09-24):能登录才允许认领/创建工件。

    复用既有加载方式(env AGENTTEAMS_ADMIN_PASSWORD 优先,其次 secrets 文件)。
    返回 (ok, sanitized_reason);reason 绝不含密码/token 内容。成功时 token
    已缓存(_TOK),后续 send 零额外登录。"""
    try:
        tok = token()
        return (bool(tok), "")
    except SystemExit as e:
        return (False, "matrix admin credential unavailable: %s" % str(e)[:120])
    except Exception as e:
        return (False, "matrix preflight failed: %s %s"
