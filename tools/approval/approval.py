...
def validate_binding_shape(b: Binding) -> None:
    ...
    if b.finding_id is not None:
        if not b.finding_id.strip():
        ...
        if b.finding_id.startswith("_"):
            # 保留命名空间:下划线前缀为 run 级哨兵('_run_')及未来内部键保留,
            # 合法 finding_id 不得进入,防两类目标在 target_key 上碰撞。
            raise ValueError(
                "BINDING: finding_id 不得使用保留前缀 '_'(内部哨兵命名空间): %r"
                % b.finding_id)
