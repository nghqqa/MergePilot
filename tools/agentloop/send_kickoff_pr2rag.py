#!/usr/bin/env python3
"""R3-TRACED kickoff: run-elem-pr2rag-20260917-01 (admin -> CoPaw leader DM).
...
RUN = "run-elem-pr2rag-20260917-01"
PROJECT = "elemiso-pr2rag-gate"

SPEC_REVIEW = (
    'pr2rag-review-1 / ' + RUN + ' - Reviewer independent security review.\n'
    ...
    '3) taskflow(submit_task) taskId "pr2rag-review-1" with result per contract (status SUCCESS/BLOCKED; summary containing '
...
KICKOFF = (
    ...
    '1. projectflow(ready_nodes) projectId "' + PROJECT + '" -> expect pr2rag-review-1.\n'
    '2. taskflow(delegate_task) projectId "' + PROJECT + '" taskId "pr2rag-review-1" roomId "room:' + TEAM_ROOM + '" '
    'spec = SPEC REVIEW below, passed EXACTLY character-for-character.\n'
    '3. Wait for the Reviewer; taskflow(check_task) taskId "pr2rag-review-1"; on acceptance mark the plan line [x].\n'
    ...
    'and WAIT for my explicit decision message here. If my decision is APPROVE: delegate pr2rag-fix-1 (taskflow, SAME rules, spec per '
    ...
    '5. After fix accepted: delegate pr2rag-verify-1 (independent clean workspace verification per contract). If VERDICT=FAIL: '
    're-delegate fix ONCE with a NEW task id pr2rag-fix-2 (never reuse a voided task id); second FAIL -> stop, block, escalate.\n'
    ...
    '=== SPEC REVIEW (pr2rag-review-1 -> @reviewer) ===\n' + SPEC_REVIEW
)
...
def main():
    ...
    with open(os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "evidence", "pr2rag", "kickoff.json"), "w") as f:
        json.dump(out, f, indent=1)
    with open(os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "evidence", "pr2rag", "kickoff-as-sent.txt"), "w", encoding="utf-8") as f:
        f.write(body)
