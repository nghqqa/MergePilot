...
GRACE_SECONDS = float(os.environ.get("AGENTLOOP_RAG_GRACE", "4"))  # 与 skills(0s)错峰:同时打补丁会互覆
...
def _apply(cfg):
    ...
    async def patched(self, *args, **kwargs):
        had = [c for c in (getattr(self, "_mcp_clients", None) or [])
               if getattr(c, "name", "") == "rag_mcp"]
        _log("register_mcp_clients ENTRY rag_present=%d" % len(had))
        try:
            if not had:
