...
- M8-A1 是 event ingestion machinery，不等于 revision producer integration；M8-A2-a 已通过隔离六容器 fixture 验证（11 PASS / 0 FAIL）；M8-A2-b 于 2026-08-18 在本机真实环境完成收口：先以 @admin producer 零种子跑通全链，再由**真实 AgentTeams Manager（hiclab v1.2.2 内 DeepSeek/OpenClaw agent）**在 operator 指令下逐字产出 M4F_RUN 契约消息（真实 Matrix event，经 Candidate 严格解析器逐字校验），真实 GitHub PR、真实 Policy Gateway 审计（run_id+git_sha provenance）、隔离 PostgreSQL 上完成 bind_revision → snapshot → 六 Skill DAG 入队；负面与幂等路径实测通过。
- AgentTeams 仍是多 Agent 协同与任务编排基座。真实 Manager producer 已在本机演示（LLM 自主决策 + 其自身 Matrix 会话发送；因果触发是 operator 的转发指令，不是自主任务分解）；Worker 侧（reviewer/fixer/verifier 的 TASK_COMPLETED handoff 回路）尚未在真实环境演示。
