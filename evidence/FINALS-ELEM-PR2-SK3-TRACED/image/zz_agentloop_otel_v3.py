...
def _start_span(name, attrs, context=None):
    t = _state.get("tracer")
    if t is None:
        return None
    try:
        return t.start_span(name, attributes=attrs, context=context)
    except Exception:
        return None
...
def _emit_delegation_link(event):
    ...
    try:
        ...
        if span is not None:
            lsc = span.get_span_context()
            _audit_write("LINK_SPAN_TRACE", "trace=%032x parent_trace=%s" % (lsc.trace_id, parts[1]))
        _end_span_ok(span)
