...
class FileSystemTaskStore:
    ...
    def _task_dir(self, task_id: str, project_id: str | None = None) -> Path:
        """Resolve a task directory.

        With ``project_id`` this is the canonical project-scoped layout
        ``shared/projects/<pid>/tasks/<tid>/`` which isolates tasks of
        different projects that share the same task_id. Without it the
        legacy flat layout ``shared/tasks/<tid>/`` is returned (read-only
        compatibility with projects created before project scoping).
        """
        if project_id:
            return self._project_dir(project_id) / "tasks" / _safe_id(task_id)
        return self.shared_dir / "tasks" / _safe_id(task_id)

    def task_shared_path(self, project_id: str, task_id: str) -> str:
        """Shared-storage prefix (for sync push/pull) of a project task."""
        return (
            f"shared/projects/{_safe_id(project_id)}/tasks/{_safe_id(task_id)}/"
        )

    def _iter_task_meta_candidates(
        self,
        task_id: str,
        project_id: str | None,
    ) -> list[tuple[Path, str, str | None]]:
        """Candidate (meta.json path, layout, candidate project) triples.

        With an explicit ``project_id`` only that project's canonical dir
        and the legacy flat dir are considered; otherwise every project's
        canonical dir is scanned so callers that do not know the project
        can still resolve uniquely-owned tasks.
        """
        candidates: list[tuple[Path, str, str | None]] = []
        if project_id:
            candidates.append(
                (self._task_dir(task_id, project_id) / "meta.json", "project", project_id),
            )
        else:
            for path in sorted(
                self.shared_dir.glob(
                    f"projects/*/tasks/{_safe_id(task_id)}/meta.json",
                ),
            ):
                # shared/projects/<pid>/tasks/<tid>/meta.json
                cand_project = path.parent.parent.parent.name
                candidates.append((path, "project", cand_project))
        candidates.append((self._task_dir(task_id) / "meta.json", "legacy", None))
        return candidates
    ...
    def _resolve_task_meta(
        ...
        """Read a task meta with cross-project isolation.

        Legacy entries whose stored ``project_id`` does not match an
        explicitly requested project are invisible (a flat ``review-1``
        left behind by another project must never satisfy this project's
        lookup). When several projects legitimately own the same task_id,
        resolution is ambiguous unless they collapse to one project.
        """
        matches: list[tuple[TaskMeta, str]] = []
        errors: list[str] = []
        for path, layout, cand_project in self._iter_task_meta_candidates(
            task_id, project_id,
        ):
            if not path.exists():
                continue
            ...
            if project_id and meta.project_id != project_id:
                continue
            matches.append((meta, layout))
        if not matches:
            scope = f" in project {project_id}" if project_id else ""
            raise TaskflowError(f"task not found: {task_id}{scope}")
        projects = {meta.project_id for meta, _ in matches}
        if len(projects) > 1:
            raise TaskflowError(
                f"task {task_id} exists in multiple projects "
                f"{sorted(projects)}; pass projectId to disambiguate",
            )
        for meta, layout in matches:
            if layout == "project":
                return meta, layout
        return matches[0]
    ...
    def read_task_meta(self, task_id: str, project_id: str | None = None) -> TaskMeta:
        meta, _layout = self._resolve_task_meta(task_id, project_id)
        return meta

    def write_task_meta(self, meta: TaskMeta) -> None:
        path = self._task_dir(meta.task_id, meta.project_id) / "meta.json"
        _write_json(path, _drop_none(asdict(meta)))

    def read_task_spec(self, task_id: str, project_id: str | None = None) -> str:
        meta, layout = self._resolve_task_meta(task_id, project_id)
        if layout == "project":
            path = self._task_dir(task_id, meta.project_id) / "spec.md"
        else:
    ...
    def write_task_spec(
        ...
        if not project_id:
            project_id = self.read_task_meta(task_id).project_id
        path = self._task_dir(task_id, project_id) / "spec.md"
        path.parent.mkdir(parents=True, exist_ok=True)
        path.write_text(spec)

    def read_task_result(self, task_id: str, project_id: str | None = None) -> TaskResult:
        meta, layout = self._resolve_task_meta(task_id, project_id)
        if layout == "project":
            path = self._task_dir(task_id, meta.project_id) / "result.md"
        else:
        ...
        validate_task_result(task_id, result, project_id=meta.project_id)
        return result
    ...
    def write_task_result(
        ...
        if not project_id:
            project_id = self.read_task_meta(task_id).project_id
        path = self._task_dir(task_id, project_id) / "result.md"
        path.parent.mkdir(parents=True, exist_ok=True)
...
def prepare_task(
    ...
    *,
    ...
    try:
        existing = store.read_task_meta(task_id, project_id=project_id)
        # A legacy flat-layout hit is not trusted when the project-scoped
        # layout is absent: delegate always writes project-scoped, so a
        # flat-only "assigned" meta is a stale pre-fix leftover and must
        # not short-circuit the claim.
    ...
    store.write_task_spec(task.task_id, spec, project_id=project_id)
...
def validate_task_result(
    ...
    if not result.summary.strip():
        raise TaskflowError("result summary is required")
    allowed_prefixes = [f"shared/tasks/{_safe_id(task_id)}/"]
    if project_id:
        allowed_prefixes.append(
            f"shared/projects/{_safe_id(project_id)}/tasks/{_safe_id(task_id)}/",
        )
        allowed_prefixes.append(
            f"shared/projects/{project_id}/tasks/{_safe_id(task_id)}/",
        )
    else:
        # Project unknown: accept any project-scoped placement of this task.
        allowed_prefixes.append(
            re.compile(rf"^shared/projects/[^/]+/tasks/{_safe_id(task_id)}/"),
        )
    for path in result.deliverables:
        ...
        if not any(
            ...
            raise TaskflowError(
                f"deliverable must be under one of "
                f"{[p if isinstance(p, str) else p.pattern for p in allowed_prefixes]}: {path}",
            )
