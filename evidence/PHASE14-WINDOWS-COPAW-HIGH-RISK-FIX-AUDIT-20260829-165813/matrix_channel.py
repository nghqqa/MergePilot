...
# Replay window: how many homeserver stream positions to rewind the
# persisted sync token on startup, so events that arrived while the
# worker was down (or its unified-queue consumer was idle-stopped) are
# re-delivered by the homeserver. The persisted seen-event ledger makes
# the redelivery exactly-once. 0 disables the rewind (legacy behaviour).
REPLAY_WINDOW_EVENTS = max(
    0,
    int(os.environ.get("AGENTTEAMS_MATRIX_REPLAY_EVENTS", "200") or "200"),
)
# Cap for the append-only seen-event ledger (one event id per line).
_SEEN_LEDGER_CAP = 10000
...
class MatrixChannel(BaseChannel):
    ...
    def __init__(
        ...
        self._seen_event_ids: set = set()  # processed-event ledger (see _record_seen_event)
    ...
    async def start(self) -> None:
        ...
        self._seen_event_ids = self._load_seen_events()
    ...
    def _load_sync_token(self) -> Optional[str]:
        ...
        For numeric stream-position tokens (Tuwunel/Continuwuity) the token
        is rewound by REPLAY_WINDOW_EVENTS positions: the sync loop advances
        the watermark as soon as a sync response arrives, independently of
        whether the downstream consumer actually processed the events, so
        restoring the raw token would permanently skip anything received
        while the worker was impaired. The seen-event ledger deduplicates
        the replayed window (exactly-once).
        """
        ...
        if path and path.exists():
            try:
                ...
                if token:
                    if REPLAY_WINDOW_EVENTS > 0 and token.isdigit():
                        rewound = max(int(token) - REPLAY_WINDOW_EVENTS, 0)
                        if rewound != int(token):
                            logger.info(
                                "MatrixChannel: rewinding sync token %s -> %s "
                                "(replay window %d)",
                                token, rewound, REPLAY_WINDOW_EVENTS,
                            )
                        return str(rewound)
                    return token
    ...
    def _seen_events_path(self) -> Optional[Path]:
        """File path of the append-only processed-event ledger."""
        wd = os.environ.get("QWENPAW_WORKING_DIR") or os.environ.get("COPAW_WORKING_DIR")
        if wd:
            return Path(wd) / "matrix_seen_events"
        return None

    def _load_seen_events(self) -> set:
        """Load the processed-event ledger (pulled from MinIO at startup)."""
        path = self._seen_events_path()
        seen: set = set()
        if path and path.exists():
            try:
                ...
                logger.warning("MatrixChannel: failed to read seen-event ledger: %s", exc)
        return seen

    def _record_seen_event(self, event_id: str) -> None:
        """Persist an event id AFTER its payload was enqueued for processing."""
        if not event_id:
            return
        ...
        try:
            ...
            with path.open("a", encoding="utf-8") as fh:
                fh.write(event_id + "\n")
            # Cap the ledger so it cannot grow without bound.
            if len(self._seen_event_ids) > _SEEN_LEDGER_CAP:
                keep = sorted(self._seen_event_ids)[-_SEEN_LEDGER_CAP:]
                path.write_text("\n".join(keep) + "\n", encoding="utf-8")
                self._seen_event_ids = set(keep)
        except Exception as exc:
            logger.warning("MatrixChannel: failed to record seen event: %s", exc)
    ...
    async def _resolve_is_dm(self, room: Any, room_id: str) -> bool:
        """Classify a room as DM, robust to unpopulated room state.

        ``room.users`` is empty (len 0) during catch-up/replay syncs until
        room state arrives; naively treating that as a group room forces
        the mention requirement and silently drops mention-less DM text
        (e.g. operator redeliveries). Query the homeserver instead.
        """
        users = getattr(room, "users", None) or {}
        count = len(users)
        if count == 2:
            return True
        if count == 0 and self._client is not None:
            try:
                ...
                logger.warning(
                    "MatrixChannel: joined_members(%s) failed: %s", room_id, exc,
                )
        return False
    ...
    async def _on_room_event(self, room: MatrixRoom, event: RoomMessageText) -> None:
        ...
        if event_id and event_id in self._seen_event_ids:
            logger.debug("_on_room_event: dedup skip %s", event_id)
            return

        sender_id = event.sender
        room_id = room.room_id
        text = event.body or ""
        is_dm = await self._resolve_is_dm(room, room_id)
        ...
        if self._enqueue:
            self._enqueue(payload)
            if event_id:
                self._record_seen_event(event_id)
            if not is_dm:
                self._clear_history(room_id)
    ...
    async def _on_room_media_event(self, room: MatrixRoom, event: Any) -> None:
        ...
        event_id = getattr(event, "event_id", None) or ""
        if event_id and event_id in self._seen_event_ids:
            logger.debug("_on_room_media_event: dedup skip %s", event_id)
            return
