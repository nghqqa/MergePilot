"""Independent verification probe for pr2r3t-verify-1.
...
def main():
    ...
    # nested dir under base, to exercise a nested "../" vector
    (base / "sub").mkdir()
    outside = root / "outside"
    outside.mkdir()
    (outside / "outside-secret.txt").write_text("TOP-SECRET-OUTSIDE-BASE", encoding="utf-8")
    # parent-level secret so "../deep-secret.txt" resolves to a real file
    ...
    cases = [
        ...
        ("leading-slash absolute",     "/etc/hostname",                   "escape"),
